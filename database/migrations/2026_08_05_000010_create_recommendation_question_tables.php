...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('answer_options', function (Blueprint $t) {
            $t->id();
            ...
            $t->unsignedSmallInteger('sort_order')->default(0);
            $t->timestamps();
            $t->unique(['question_id', 'value']);
        });
        Schema::create('recommendation_rules', function (Blueprint $t) {
            $t->id();
            $t->foreignId('answer_option_id')->constrained()->cascadeOnDelete();
            $t->string('criterion', 20);
            $t->unsignedBigInteger('target_id')->nullable();
            $t->string('target_value')->nullable();
            $t->unsignedTinyInteger('weight')->default(10);
            $t->boolean('is_active')->default(true);
            $t->timestamps();
            $t->index(['criterion', 'target_id']);
        });
    }
